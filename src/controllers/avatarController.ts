...
type RawAvatarRow = {
  ...
  option_ability: string | null;
  usage_count:    string;          // still string coming from PG
};
...
export const getAvatar = async (
  ...
  try {
    ...
    `;
    const charResult = await client.query<{ character_id: string }>(
      charQuery,
    ...
    for (const { character_id } of charResult.rows) {
      ...
      let apiResp;
      try {
        apiResp = await axios.get<{ avatar?: AvatarSlot[] }>(url);
      } catch (err: any) {
      ...
      for (const slot of avatarData) {
        const {
          slotId, slotName, itemId, itemName,
          itemRarity, optionAbility = null, emblems
        } = slot;
        ...
        let ins;
        try {
          ins = await client.query<{ id: number }>(upsertAvatar, [
        ...
        // Clear old emblems
        await client.query(
          `DELETE FROM character_avatar_emblems WHERE character_avatar_id = $1;`,
          [avatarRecordId]
        ).catch(err => {
            ...
            try {
              await client.query(
                `INSERT INTO character_avatar_emblems
                  ...
                  VALUES ($1,$2,$3,$4,$5,$6);`,
                [avatarRecordId, slotNo, slotColor, eid, ename, erarity]
              );
            } catch (err: any) {
...
export const getAvatarStats = async (
  ...
  try {
    ...
    `;
    const wepRes = await client.query<RawAvatarRow>(wepQuery, [jobId, jobGrowId]);
    ...
    `;
    const optRes = await client.query<RawAvatarRow>(optQuery, [jobId, jobGrowId]);
    ...
    const emblemQuery = `
      ...
      ORDER BY
        ...
        usage_count DESC;
    `;
    const emblemRes = await client.query<RawEmblemRow>(emblemQuery, [jobId, jobGrowId]);

    // Merge avatar rows
    const rawAvatar: RawAvatarRow[] = [...wepRes.rows, ...optRes.rows];
    const avatarStatsBySlot: Record<string, AvatarStat[]> = {};

    for (const row of rawAvatar) {
      const count = parseInt(row.usage_count, 10);
      avatarStatsBySlot[row.slot_id] ??= [];
    ...
    // Process emblems
    const emblemStatsByColor: Record<string, EmblemStat[]> = {};
