...
export const fetchEquipment = async (
  ...
  try {
    ...
    for (const { character_id } of charRes.rows) {
      ...
      let apiRes;
      try {
        apiRes = await axios.get<{ equipment?: EquipmentApiSlot[] }>(url);
      } catch (err: any) {
...
export const getEquipmentStats = async (
  ...
  try {
    ...
    `;
    const titleRes = await client.query<TitleRow>(titleQ, [jobId, jobGrowId]);
    ...
    `;
    const itemsRes = await client.query<ItemRow>(itemsQ, [jobId, jobGrowId]);
    ...
    const fusionQ = `
      ...
      ORDER BY slot_id, usage_count DESC;
    `;
    const fusionRes = await client.query<FusionRow>(fusionQ, [jobId, jobGrowId]);
    ...
    const setQ = `
      ...
      ORDER BY usage_count DESC;
    `;
    const setRes = await client.query<SetRow>(setQ, [jobId, jobGrowId]);
    ...
    const sampleQ = `
      ...
      GROUP BY slot_id;
    `;
    const sampleRes = await client.query<SampleRow>(sampleQ, [jobId, jobGrowId]);
    const sampleNumbers: Record<string, number> = {};
    ...
    const fusionSampleQ = `
      ...
      GROUP BY slot_id;
    `;
    const fusionSampleRes = await client.query<SampleRow>(fusionSampleQ, [jobId, jobGrowId]);
    const fusionSampleNumbers: Record<string, number> = {};
    ...
    const itemsBySlot: Record<string, typeof itemsStats> = {};
    const fusionBySlot: Record<string, typeof fusionStats> = {};
    ...
    itemsStats.forEach(s => {
      if (itemsBySlot[s.slot]) itemsBySlot[s.slot].push(s);
    });
    fusionStats.forEach(s => {
      if (fusionBySlot[s.slot]) fusionBySlot[s.slot].push(s);
    });
    ...
    const setSampleQ2 = `
      ...
      WHERE ce.slot_id<>'WEAPON'
        ...
        AND c.job_id=$1 AND c.job_grow_id=$2;
    `;
    const setSampleRes2 = await client.query<{ sample_number: string }>(
      setSampleQ2,
      [jobId, jobGrowId]
    );
...
// -------------------
export const getEquipmentCombinations = async (
  req: Request<{ jobId: string; jobGrowId: string }>,
  res: Response
): Promise<void> => {
  const { jobId, jobGrowId } = req.params;
  ...
  const makeComboQ = (slots: string[], useFusion = false) => {
  ...
  try {
    ...
    for (const [group, slots] of Object.entries(combos)) {
      ...
      const { rows: normalRows } = await client.query(normalQ, [jobId, jobGrowId]);
      results[group] = normalRows;

      const fusionQ = makeComboQ(slots, true);
      const { rows: fusionRows } = await client.query(fusionQ, [jobId, jobGrowId]);
