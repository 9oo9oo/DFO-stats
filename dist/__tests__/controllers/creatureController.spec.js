...
const { fetchCreature, getCreatureStats, getCreatureArtifactCombinations } = require('../../controllers/creatureController');
...
describe('creatureController.getCreatureArtifactCombinations', () => {
    ...
    it('handles combo DB error', async () => {
        ...
        await getCreatureArtifactCombinations(req, res);
        expect(console.error).toHaveBeenCalledWith('Error fetching creature-artifact combos:', expect.any(Error));
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.json).toHaveBeenCalledWith({ error: 'Internal Server Error' });
    });
});
