...
describe('models/db', () => {
    ...
    it('should call connect() and query("SELECT version();") on import', async () => {
        ...
        expect(console.log).toHaveBeenCalledWith('PostgreSQL version:', { version: 'PostgreSQL 13.4' });
    });
...
describe('models/db connection error', () => {
    ...
    it('logs error when client.connect() rejects', async () => {
        ...
        await jest.runAllTimersAsync();
        expect(consoleError).toHaveBeenCalledWith('Connection error:', expect.any(String));
    });
});
